...
if [ "$mode" = "name" ]; then
    ...
    kitty --class pdfsearch sh -c "true | fzf --prompt='🔍 Search PDFs: ' --bind \"change:reload:if [ -n {q} ]; then rga --type pdf --no-heading --color=always {q} $directories; else true; fi\" --ansi > $tmpfile"
